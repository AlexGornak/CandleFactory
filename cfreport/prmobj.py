...
class RegPoint(object):
    def __init__(self, _id, _name, _desc, _caddr, _status):
        ...
        self.A, self.kt, self.kn, self.kc, self.Ci=1250, 1, 1, 1, 1
        self.c_pass=''
        self.mon_data=dict()
        self.iter=None
        self.chiters=dict()
...
class RegValue(object):
    def __init__(self, _id, _vname, _vshortname, _vunit, _vmon, _vmcmd_id):
        self.id, self.name, self.short_name, self.unit, self._mon, self.mcmd_id = \
                 _id, _vname, _vshortname, _vunit, _vmon, _vmcmd_id
    ...
    #def load_from_db(self):
        for row in SQL_DB.select(SQL_SEL_REG_VALUES):
            self.append(RegValue(row['v_id'], row['v_name'], row['v_short_name'], \
                                 row['v_unit'], row['v_mon'], row['v_mcmd_id']))
